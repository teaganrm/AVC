    ...
    if (not aligned[0]):
        ...
        if (distance < 1500 and distance > 50): # if aligned and close, perform action
            ...
            while time.time() - startTime < 2:
                print("Preparing to turn around blue")
...
def yellow_bucket_function(center_x, distance):
    ...
    if (not aligned[0]):
        ...
        if (distance < DISTANCE_THRESHOLD): # if aligned and close, perform action
            serial_port.write(NEUTRAL)
            while time.time() - startTime < 2:
                print("Preparing to turn around yellow")
            serial_port.write(LEFT45)
            serial_port.write(FORWARD20)
            startTime = time.time()
            while time.time() - startTime < 0.2:
            ...
            startTime = time.time()
            while time.time() - startTime < 4: # go forward 30% for 0.5 seconds (45% right)
                print("Turning around yellow")
            
            serial_port.write(LEFT68)
            startTime = time.time()
            while time.time() - startTime < 6: # go forward 30% for 2 seconds (23% left)
                print("Turning around yellow")
